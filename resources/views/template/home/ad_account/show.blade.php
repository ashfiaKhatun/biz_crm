                        ...
                        <div class="card-body">
                            ...
                            <div class="row">
                                ...
                                <form action="{{ route('ad-account.updateStatus', $adAccount->id) }}" method="post">
                                    @csrf
                                    @method('PATCH')
                                    <select name="status" class="form-select-sm rounded" onchange="this.form.submit()">
                                        <option class="{{ $adAccount->status == 'in-review' || $adAccount->status == 'canceled' || $adAccount->status == 'approved' || $adAccount->status == 'rejected' ? 'd-none' : '' }}" value="pending" {{ $adAccount->status == 'pending' ? 'selected' : '' }}>
                                            Pending
                                        </option>
                                        ...
                                        <option class="{{ $adAccount->status == 'canceled' || $adAccount->status == 'rejected' ? 'd-none' : '' }}" value="approved" {{ $adAccount->status == 'approved' ? 'selected' : '' }}>
                                            Approved
                                        </option>
                                        <option class="{{ $adAccount->status == 'approved' || $adAccount->status == 'rejected' ? 'd-none' : '' }}" value="canceled" {{ $adAccount->status == 'canceled' ? 'selected' : '' }}>
                                            Canceled
                                        </option>
                                        <option class="{{ $adAccount->status == 'approved' || $adAccount->status == 'canceled' ? 'd-none' : '' }}" value="rejected" {{ $adAccount->status == 'rejected' ? 'selected' : '' }}>
                                            Rejected
                                        </option>
                                    </select>
                                </form>
